...
//直线和线段关系:0 平行,1 重合,2 相交
int SegLineCross(Point a,Point b,Point c,Point d){
	int d1=sgn(Cross(a-c,b-c));
	int d2=sgn(Cross(a-d,b-d));
	if((d1^d2)==-2) return 1;
...
Point get_circle(Point p1,Point p2){
    Point mid=Point((p1.x+p2.x)/2,(p1.y+p2.y)/2);//中点
    double angle=atan2(p1.x-p2.x,p2.y-p1.y);//角度
...
int main(){
    ...
    for(i=0;i<n;i++){
        for(j=i+1;j<n;j++){
            ...
            Point center=get_circle(p[i],p[j]);//求圆心
            int cnt=0;
            for(int k=0;k<n;k++){
