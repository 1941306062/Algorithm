...
///flush(stdout);  刷新输出缓冲区与cout<<endl;相同
...
freopen("C:\\Users\\ASUS\\Desktop\\Test data\\retribution\\data\\secret\\r01.txt","r",stdin); //输入重定向，输入数据将从in.txt文件中读取
//
freopen("C:\\Users\\ASUS\\Desktop\\Test data\\retribution\\data\\secret\\out.txt","w",stdout); //输出重定向，输出数据将保存在out.txt文件中
   fclose(stdin);//关闭文件
    fclose(stdout);//关闭文件
#endif
vector<int>v;
